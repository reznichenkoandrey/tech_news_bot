...
async function maybeTriggerDigest(env: Env): Promise<void> {
  ...
  // Auth is a long-lived setup-token in the GitHub Secret
  // CLAUDE_CODE_OAUTH_TOKEN; the workflow reads it directly. The worker only
  // needs to fire the dispatch — no OAuth refresh happens here anymore.
  ...
  if (res.ok) {
    console.log("scheduled: dispatch queued");
    return;
  }
